...
async function scout(path, timeoutMs = 5000) {
  const ctrl = new AbortController();
  const t = setTimeout(() => ctrl.abort(), timeoutMs);
  try {
...
export async function getToken(addr) {
  ...
  const token = {
    ...
    decimals: Number((meta && meta.decimals) || 18),
    totalSupply: (meta && meta.total_supply) || null,
  ...
  const holders = meta && (meta.holders || meta.holder_count) ? Number(meta.holders || meta.holder_count) : null;
  return { token, market, liquidity, holdersCount: holders };
}
...
export async function getHolders(addr, totalSupplyStr) {
  ...
  const holders = items.slice(0, 50).map(h => {
    ...
    const a = (h.address && h.address.hash) || h.address_hash || h.address || '';
    return { addr: String(a).toLowerCase(), pct };
  }).filter(h => h.pct > 0);

  const top1 = holders[0] ? holders[0].pct : 0;
  const top10 = holders.slice(0, 10).reduce((s, h) => s + h.pct, 0);
  return { list: holders, count: holders.length, top1Pct: round2(top1), top10Pct: round2(top10) };
}
