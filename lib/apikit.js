...
// edge and node runtimes — node:crypto is not supported on edge.
...
export async function authKey(req) {
  ...
  if (!raw) return null;
  try {
    const kh = await hashKey(raw);
    const rec = await kv.get('key:' + kh);
    if (!rec || rec.active === false) return null;
    return { owner: rec.owner || 'unknown', tier: rec.tier || 'free', keyhash: kh };
  } catch (e) {
    return null;
  }
}
